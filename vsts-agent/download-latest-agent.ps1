...
function downloadFile($url, $targetFile)
{
    ...
    while ($count -gt 0) 
    {
        ...
        Write-Progress -activity "Downloading file '$($url.split('/') | Select -Last 1)' to location '$($targetFile)'" -status "Downloaded ($([System.Math]::Floor($downloadedBytes/1024))K of $($totalLength)K): " -PercentComplete ((([System.Math]::Floor($downloadedBytes/1024)) / $totalLength)  * 100)
    } 
...
downloadFile "$latestAgentUrl" "$PSScriptRoot\agent.zip"